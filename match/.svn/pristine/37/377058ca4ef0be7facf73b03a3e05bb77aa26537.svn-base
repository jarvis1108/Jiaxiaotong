<template>
	<div class="st-container-scholarship-div">
		...
		<div class="st-productList-div">
          ...
          <div v-if="index%3 === 0" class="st-goodPhoto-blue-div">
            <img class="st-goodPhoto-img" :src="item.scholarshipImage" @click="exchangeScholarship(item)"/>
          </div>
...
export default {
  ...
  methods: {
    async exchangeScholarship(item) {
      console.log("222");
      await this.$store.dispatch("exchangeScholarship",item.scholarshipId);
    }
  },
  async onLoad() {
    this.productList = await this.$store.dispatch("getProductListOfScholarship");
  }
};
</script>
