 ...
 *              <br/> //  *          （1）本文件的接口清单如下：
 *              <br/> //  *              （a）ExchangeLearningPoint：兑换学点
...
/**
 * @class
 * @summary 实现功能：兑换学点用例中接口定义
 * @memberOf MAL
 * @description                           
 */
function MAL_CHG_010() {

    /**
     * @summary 实现功能：用户利用金币兑换学点
     * @description **POST** /api/v1/mall/productInfo/exchange
     * @param {object} objParam                                        要传入的参数
     * @param {string} objParam.reqId                                  要传入的参数.请求的Id
     * @param {string} objParam.reqType                                要传入的参数.请求类型 固定值为: MAL_CHG_010_ChangeLearningPoint
	 ...
	 * @param {string} objParam.reqParam.productId                     要传入的参数.请求的业务参数.商品Id
	 * @param {number} objParam.reqParam.amount                        要传入的参数.请求的业务参数.商品数量
     ...
     * @param {boolean} objResult.resResult.isSuccess                     返回结果.响应返回的业务数据.响应是否成功
     ...
     * @example 暂无
     */
    function ExchangeLearningPoint(objParam) {
        var objResult = {};
        return objResult;
    }
}