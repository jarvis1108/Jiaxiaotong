...
export default {
  ...
  async onLoad(option) {
    ...
    if (option.matchId) {
      ...
      //若参数中没有战局信息，则请求后台创建一个战局，且当前用户为创建者
      this.isCreator = true;
  ...
  methods: {
    ...
    receiveMessage(res) {
      ...
      } else if (result.msgType === "Z-1") {
        //当前战局达到最大人数
        wxs.showTip("当前战局已达到最大人数");
        setTimeout(() => {
          wxs.navigateBack(0);
        }, 2000);
      } else if (result.msgType === "Z-2") {
        //战局创建者已退出，直接返回首页
        if (this.questionIndex < 6) {
          wxs.showTip("战局已解散，返回首页");
          setTimeout(() => {
            wxs.navigateBack(0);
          }, 2000);
        }
      } else if (result.msgType === "Z-3") {
        //非战局创建者退出，销毁对手信息
        if (this.questionIndex < 6) {
          wxs.showTip("好友退出战局");
          this.friendInfo = {};
    ...
    sendMessage() {
      ...
      } else if (this.waitUser && this.waitOpponent && this.questionIndex < 5) {
