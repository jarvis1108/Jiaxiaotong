...
function MAL_RNG_020() {
     ...
     * @param {string} objParam.reqType                                要传入的参数.请求类型 固定值为: MAL_RNG_020_RechargeLearingPoint
     ...
     * @param {object} objParam.reqParam.openid                        要传入的参数.请求的业务参数.openid
     ...
     * @param {object} objResult.resResult.errorInfo                      返回结果.响应返回的业务数据.出错信息(具体结构框架统一定义)
     * @param {object} objResult.resRefData                               返回结果.存放业务以外的其它信息(预留扩展，一般情况下不用) default: {}
     * @version 1.0
     * @example 暂无
     */
    function RechargeLearingPoint(objParam) {
        var objResult = {};
        return objResult;
    }
}