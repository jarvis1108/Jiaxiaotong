...
BEGIN 
  ...
  read_loop: LOOP
    ...
    END IF;
	  ...
	  INSERT INTO QuestionBankTemp.t_ITN_Knowledge(  KnowledgeID,TreeNodeNo,InitID,InitNo,IsCorrect,															  KnowledgeParentID,SubjectID,Content,NodeLevel,IsLeaf,
															  CreateTime,CreateUser,LastUpdTime,LastUpdUser,IsValid,
																Remark,FromCrawlerKnowledgeID)
		SELECT D_KnowledgeID,D_TreeNodeNo,D_InitID,D_InitNo,D_IsCorrect,														  
					 D_KnowledgeParentID, D_SubjectID,D_Content,D_NodeLevel,D_IsLeaf,
					 D_CreateTime,D_CreateUser,D_LastUpdTime,
					 D_LastUpdUser,D_IsValid,D_Remark,S_KnowledgeID;
  END LOOP;
...
BEGIN
	 ...
	 DECLARE strSQLText	  VARCHAR(8000);
   ...
   DELETE From QuestionBankTemp.t_ITN_Question
	 ...
	 INSERT Into QuestionBankTemp.t_ITN_Main
		...
		SELECT In_Str_InitID As InitID, In_Str_InitID As InitNo,null As InitUserCode,null AS InitUserName,
					 CONCAT(date_format( now( ), '%Y-%m-%d %H:%m:%s' ),"迁移数据的数据库初始化") As InitName,
					 CONCAT(date_format( now( ), '%Y-%m-%d %H:%m:%s' ),"迁移数据的数据库初始化") As InitDes,						
   ...
   /*	 
	 ...
	 (
		...
		*/
		...
		-- 将数据插入临时表
		PREPARE SqlText FROM 'Insert QuestionBankTemp.Tmp_Tb_Question(QuestionID,InitID,InitNo,KnowledgeID,Question,QuestionTxt,
																									 ChoiceA,ChoiceB,ChoiceC,ChoiceD,Answer,
																									 QuestionAnalyze,Difficulty,FromQuestionID)
														SELECT UUID(),InitID,InitNo,KnowledgeID,
																	 Question,QuestionTxt,ChoiceA,
																	 ChoiceB,ChoiceC,ChoiceD,Answer,QuestionAnalyze,
																	 Difficulty,FromQuestionID
   ...
   PrsLooP:LOOP		
	  ...
	  Set nBeginIndex = nCurBatIndex * In_N_BatCount;
		IF nBeginIndex >= NRecordCount   THEN
			LEAVE PrsLooP; 
		END IF;
