...
CREATE TABLE `t_ITN_MainManual`  (
  ...
  `InitNo` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '初始化编码',
  ...
  `InitTime` timestamp(0) NULL DEFAULT NULL COMMENT '录入时间',
  ...
  `LastUpdTime` timestamp(0) NULL DEFAULT NULL COMMENT '上次更新时间',
...
CREATE TABLE `t_ITN_QuestionManual`  (
  ...
  `ExaminationVersion` varchar(128) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '试卷版本号',
  ...
  `KnowledgeDes` varchar(128) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '知识点说明，科目_知识点,如：一年级数学下册_第二单元/认识前后',
  `Question` varchar(512) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '题干',
  ...
  `QuestionAnalyze` varchar(1024) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '题目解析',
  ...
  `PrsTime` timestamp(0) NULL DEFAULT NULL COMMENT '处理时间',
  ...
  `CheckTime` timestamp(0) NULL DEFAULT NULL COMMENT '审核时间',
  `IsValid` tinyint(4) NULL DEFAULT NULL COMMENT '是否有效，1表示有效，0表示无效',
  `Remark` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '备注信息',
...
CREATE TABLE `t_ITN_QuestionMultipleChoiceManual`  (
  ...
  `Answer` varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '答案：0:A; 1B; 2:C; 3:D',
  `CreateTime` timestamp(0) NULL DEFAULT NULL COMMENT '录入时间',
...
-- ----------------------------
-- View structure for Dvlp_v_Init_KnowledgeManual
-- ----------------------------
DROP VIEW IF EXISTS `Dvlp_v_Init_KnowledgeManual`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `Dvlp_v_Init_KnowledgeManual` AS select `t1`.`KnowledgeID` AS `KnowledgeID`,`t1`.`InitID` AS `InitID`,`t1`.`InitNo` AS `InitNo`,`t1`.`SubjectNameCode` AS `SubjectNameCode`,`t1`.`OrderIndex` AS `OrderIndex`,`t1`.`TreeCode` AS `TreeCode`,`t1`.`IsCorrect` AS `IsCorrect`,`t1`.`Content` AS `Content`,`t1`.`Des` AS `Des`,`t1`.`CreateTime` AS `CreateTime`,`t1`.`CreateUser` AS `CreateUser`,`t1`.`LastUpdTime` AS `LastUpdTime`,`t1`.`LastUpdUser` AS `LastUpdUser`,`t1`.`Remark` AS `Remark`,`t1`.`IsValid` AS `IsValid`,`t2`.`SubjectID` AS `SubjectID` from (`QuestionBankManual`.`t_ITN_KnowledgeManual` `t1` left join `QuestionBankTemp`.`t_ITN_Subject` `t2` on((locate(`t2`.`SubjectName`,`t1`.`SubjectNameCode`) > 0)));

-- ----------------------------
-- View structure for Dvlp_v_Init_Subject1Manual
-- ----------------------------
DROP VIEW IF EXISTS `Dvlp_v_Init_Subject1Manual`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `Dvlp_v_Init_Subject1Manual` AS select `V`.`SubjectID` AS `SubjectID`,`V`.`InitID` AS `InitID`,`V`.`InitNo` AS `InitNo`,`V`.`IsCorrect` AS `IsCorrect`,`V`.`OrderIndex` AS `OrderIndex`,`V`.`TreeCode` AS `TreeCode`,`V`.`SubjectName` AS `SubjectName`,`V`.`SubjectShortName` AS `SubjectShortName`,`V`.`CreateTime` AS `CreateTime`,ifnull(`V`.`CreateUser`,`StrongestScholar`.`Dvlp_Fn_GetDefaultUserID`('A')) AS `CreateUser`,`V`.`LastUpdTime` AS `LastUpdTime`,ifnull(`V`.`LastUpdUser`,`StrongestScholar`.`Dvlp_Fn_GetDefaultUserID`('A')) AS `LastUpdUser`,`V`.`IsValid` AS `IsValid`,`V`.`Remark` AS `Remark`,substring_index(`V`.`TreeCode`,'.',1) AS `TreeCodeFirst`,(`StrongestScholar`.`Dvlp_Fn_GetSubstrCount`(`V`.`TreeCode`,'.') + 1) AS `TreeLevel` from `t_ITN_SubjectManual` `V`;

-- ----------------------------
-- View structure for Dvlp_v_Init_Subject2Manual
-- ----------------------------
DROP VIEW IF EXISTS `Dvlp_v_Init_Subject2Manual`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `Dvlp_v_Init_Subject2Manual` AS select `V`.`SubjectID` AS `SubjectID`,`V`.`InitID` AS `InitID`,`V`.`InitNo` AS `InitNo`,`V`.`IsCorrect` AS `IsCorrect`,`V`.`OrderIndex` AS `OrderIndex`,`V`.`TreeCode` AS `TreeCode`,`V`.`SubjectName` AS `SubjectName`,`V`.`SubjectShortName` AS `SubjectShortName`,`V`.`CreateTime` AS `CreateTime`,`V`.`CreateUser` AS `CreateUser`,`V`.`LastUpdTime` AS `LastUpdTime`,`V`.`LastUpdUser` AS `LastUpdUser`,`V`.`IsValid` AS `IsValid`,`V`.`Remark` AS `Remark`,`V`.`TreeCodeFirst` AS `TreeCodeFirst`,`V`.`TreeLevel` AS `TreeLevel`,(case `V`.`TreeCode` when '6.6' then 1 when '6.7' then 1 when '9.10' then 1 when '9.11' then 1 when '10.10' then 1 when '10.11' then 1 else (`V`.`TreeLevel` = 3) end) AS `IsLeaf`,(case `V`.`TreeCodeFirst` when '1' then '小学' when '2' then '小学' when '3' then '小学' when '4' then '小学' when '5' then '小学' when '6' then '小学' when '7' then '初中' when '8' then '初中' when '9' then '初中' when '10' then '高中' when '15' then '大学' end) AS `GradePhaseName`,(case `V`.`TreeCodeFirst` when '1' then '一年级' when '2' then '二年级' when '3' then '三年级' when '4' then '四年级' when '5' then '五年级' when '6' then (case `V`.`TreeCode` when '6.6' then NULL when '6.7' then NULL else '六年级' end) when '7' then '七年级' when '8' then '八年级' when '9' then (case `V`.`TreeCode` when '9.10' then NULL when '9.11' then NULL else '九年级' end) end) AS `GradeName`,(case `V`.`TreeCodeFirst` when '1' then substr(`V`.`SubjectName`,4,2) when '2' then substr(`V`.`SubjectName`,4,2) when '3' then substr(`V`.`SubjectName`,4,2) when '4' then substr(`V`.`SubjectName`,4,2) when '5' then substr(`V`.`SubjectName`,4,2) when '6' then (case `V`.`TreeCode` when '6.6' then substr(`V`.`SubjectName`,3,2) when '6.7' then substr(`V`.`SubjectName`,3,2) else substr(`V`.`SubjectName`,4,2) end) when '7' then substr(`V`.`SubjectName`,4,2) when '8' then substr(`V`.`SubjectName`,4,2) when '9' then (case `V`.`TreeCode` when '9.10' then substr(`V`.`SubjectName`,3,2) when '9.11' then substr(`V`.`SubjectName`,3,2) else substr(`V`.`SubjectName`,4,2) end) when '15' then convert((case substr(`V`.`TreeCode`,1,4) when '15.1' then '英语' when '15.2' then '政治' else NULL end) using utf8) else substr(`V`.`SubjectName`,3,2) end) AS `CourseName`,(case `V`.`TreeCodeFirst` when '1' then substr(`V`.`SubjectName`,6) when '2' then substr(`V`.`SubjectName`,6) when '3' then substr(`V`.`SubjectName`,6) when '4' then substr(`V`.`SubjectName`,6) when '5' then substr(`V`.`SubjectName`,6) when '6' then (case `V`.`TreeCode` when '6.6' then substr(`V`.`SubjectName`,5) when '6.7' then substr(`V`.`SubjectName`,5) else substr(`V`.`SubjectName`,6) end) when '7' then substr(`V`.`SubjectName`,6) when '8' then substr(`V`.`SubjectName`,6) when '9' then (case `V`.`TreeCode` when '9.10' then substr(`V`.`SubjectName`,5) when '9.11' then substr(`V`.`SubjectName`,5) else substr(`V`.`SubjectName`,6) end) when '10' then (case `V`.`TreeCode` when '10.10' then substr(`V`.`SubjectName`,5) when '10.11' then substr(`V`.`SubjectName`,5) else substr(`V`.`SubjectName`,5,2) end) end) AS `Booklet` from `Dvlp_v_Init_Subject1Manual` `V`;

-- ----------------------------
-- View structure for Dvlp_v_Init_Subject3Manual
-- ----------------------------
DROP VIEW IF EXISTS `Dvlp_v_Init_Subject3Manual`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `Dvlp_v_Init_Subject3Manual` AS select `V1`.`SubjectID` AS `SubjectID`,`V1`.`InitID` AS `InitID`,`V1`.`InitNo` AS `InitNo`,`V1`.`IsCorrect` AS `IsCorrect`,`V1`.`OrderIndex` AS `OrderIndex`,`V1`.`TreeCode` AS `TreeCode`,`V1`.`SubjectName` AS `SubjectName`,`V1`.`SubjectShortName` AS `SubjectShortName`,`V1`.`CreateTime` AS `CreateTime`,`V1`.`CreateUser` AS `CreateUser`,`V1`.`LastUpdTime` AS `LastUpdTime`,`V1`.`LastUpdUser` AS `LastUpdUser`,`V1`.`IsValid` AS `IsValid`,`V1`.`Remark` AS `Remark`,`V1`.`TreeCodeFirst` AS `TreeCodeFirst`,`V1`.`TreeLevel` AS `TreeLevel`,`V1`.`IsLeaf` AS `IsLeaf`,`V1`.`GradePhaseName` AS `GradePhaseName`,`V1`.`GradeName` AS `GradeName`,`V1`.`CourseName` AS `CourseName`,`V1`.`Booklet` AS `Booklet`,(select `TX1`.`GradePhaseID` from `StrongestScholar`.`t_BSD_GradePhaseInfo` `TX1` where (convert(`V1`.`GradePhaseName` using utf8) = `TX1`.`GradePhaseName`)) AS `GradePhaseID`,(select `TX2`.`GradeID` from `StrongestScholar`.`t_BSD_GradeInfo` `TX2` where (convert(`V1`.`GradeName` using utf8) = `TX2`.`GradeName`)) AS `GradeID`,(select `TX3`.`CourseID` from `StrongestScholar`.`t_BSD_CourseInfo` `TX3` where (`V1`.`CourseName` = `TX3`.`CourseName`)) AS `CourseID` from `QuestionBankManual`.`Dvlp_v_Init_Subject2Manual` `V1`;
