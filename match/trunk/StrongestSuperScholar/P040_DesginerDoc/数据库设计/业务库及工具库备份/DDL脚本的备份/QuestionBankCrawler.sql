...
CREATE TABLE `tb_question`  (
  ...
  `knowledgeID` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '知识点的ID，指爬虫数据库中tb_knowledge的id字段',
...
-- ----------------------------
-- Table structure for tb_question_test
-- ----------------------------
DROP TABLE IF EXISTS `tb_question_test`;
CREATE TABLE `tb_question_test`  (
  `initId` varchar(36) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '初始化ID',
  `id` varchar(36) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '试题id',
  `InitNo` varchar(36) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '初始化编码',
  `title` text CHARACTER SET utf8 COLLATE utf8_general_ci NULL COMMENT '题干',
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `Dvlp_Init_V_Question` AS select `t1`.`initId` AS `InitID`,`t1`.`InitNo` AS `InitNo`,`t1`.`phase` AS `phase`,`t3`.`KnowledgeID` AS `KnowledgeID`,`t1`.`title` AS `Question`,`t1`.`titleTxt` AS `QuestionTxt`,`t1`.`choiceA` AS `ChoiceA`,`t1`.`choiceB` AS `ChoiceB`,`t1`.`choiceC` AS `ChoiceC`,`t1`.`choiceD` AS `ChoiceD`,`t1`.`answer` AS `Answer`,ifnull(`t1`.`status`,0) AS `status`,`t1`.`analysis` AS `QuestionAnalyze`,`t1`.`difficulty` AS `Difficulty`,`t1`.`id` AS `FromQuestionID` from (`QuestionBankCrawler`.`tb_question` `t1` left join `QuestionBankTemp`.`t_ITN_Knowledge` `t3` on((`t1`.`knowledgeID` = `t3`.`FromCrawlerID`))) where ((`t1`.`bookVersion` in ('人教版','初中语文基础知识','初中英语基础知识','高中语言基础知识','高中英文基础知识')) and (`t1`.`phase` in ('初中','高中')));
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `Dvlp_‌Init_V_Knowledge` AS select `T1`.`id` AS `id`,`T1`.`subject_id` AS `subject_id`,`T1`.`subject_name` AS `subject_name`,`T1`.`version_id` AS `version_id`,`T1`.`version_name` AS `version_name`,`T1`.`grade_id` AS `grade_id`,`T1`.`grade_name` AS `grade_name`,replace((case substr(`T1`.`subject_name`,1,2) when '高中' then concat(`T1`.`subject_name`,`T1`.`grade_name`) else concat(substr(`T1`.`grade_name`,1,3),substr(`T1`.`subject_name`,3,2),substr(`T1`.`grade_name`,4)) end),'思品','政治') AS `grade_name_AOgr`,`T2`.`SubjectName` AS `grade_name_A`,`T1`.`unit_id` AS `unit_id`,`T1`.`unit_name` AS `unit_name`,`T1`.`chapter_id` AS `chapter_id`,`T1`.`chapter_name` AS `chapter_name`,`T1`.`knowledge_id` AS `knowledge_id`,`T1`.`knowledge_name` AS `knowledge_name`,`T1`.`page_num` AS `page_num`,`T1`.`is_finish` AS `is_finish`,`T2`.`SubjectID` AS `D_SubjectID`,`T2`.`SubjectNo` AS `D_SubjectNo`,`T2`.`SubjectName` AS `D_SubjectName`,`T2`.`GradePhaseID` AS `D_GradePhaseID`,`T2`.`GradeID` AS `D_GradeID`,`T2`.`CourseID` AS `D_CourseID`,substr(`T1`.`subject_name`,1,2) AS `phaseInQ`,(case `T1`.`grade_name` when '基础知识' then '基础知识' else substr(`T1`.`grade_name`,1,3) end) AS `gradeInQ`,`T1`.`version_name` AS `bookVersionInQ`,substr(`T1`.`subject_name`,3) AS `subjectInQ`,(case `T1`.`grade_name` when '基础知识' then NULL else (case substr(`T1`.`subject_name`,1,2) when '高中' then `T1`.`grade_name` else substr(`T1`.`grade_name`,4) end) end) AS `bookletInQ` from (`QuestionBankCrawler`.`tb_knowledge` `T1` left join `QuestionBankTemp`.`t_ITN_Subject` `T2` on((replace((case substr(`T1`.`subject_name`,1,2) when '高中' then concat(`T1`.`subject_name`,`T1`.`grade_name`) else (case `T1`.`grade_name` when '基础知识' then concat(`T1`.`subject_name`,`T1`.`grade_name`) else concat(substr(`T1`.`grade_name`,1,3),substr(`T1`.`subject_name`,3,2),substr(`T1`.`grade_name`,4)) end) end),'思品','政治') = `T2`.`SubjectName`))) where ((`T1`.`version_name` in ('人教版','初中语文基础知识','初中英语基础知识','高中语文基础知识','高中英文基础知识')) and ((`T1`.`subject_name` like '初中%') or (`T1`.`subject_name` like '高中%')));

-- ----------------------------
-- Function structure for Dvlp_Fn_GetQuestionKnowledgeID
-- ----------------------------
DROP FUNCTION IF EXISTS `Dvlp_Fn_GetQuestionKnowledgeID`;
delimiter ;;
CREATE FUNCTION `Dvlp_Fn_GetQuestionKnowledgeID`(phase VARCHAR(255),
			subject VARCHAR(255),
			grade VARCHAR(255),
			booklet VARCHAR(255),
			bookVersion VARCHAR(255),
			unit VARCHAR(255),
			chapter VARCHAR(255),
			knowledge VARCHAR(255))
 RETURNS varchar(36) CHARSET utf8
BEGIN
	DECLARE knowledgeID VARCHAR(36) DEFAULT NULL;
	DECLARE counts INT DEFAULT 0;
	SELECT MAX(id),COUNT(1)INTO knowledgeID,counts
	FROM QuestionBankCrawler.tb_knowledge t2
	WHERE ( concat( phase, subject ) = t2.subject_name ) 
				 AND (
						concat(
							( CASE grade WHEN NULL THEN '基础知识' WHEN '' THEN '基础知识' ELSE grade END ),
							ifnull( booklet, '' ) 
						) = t2.grade_name
					) 
					AND ( bookVersion = t2.version_name ) 
					AND ( unit = t2.unit_name ) 
					AND ( chapter = t2.chapter_name ) 
					AND ( knowledge = t2.knowledge_name );
		IF counts=0 THEN 
			 RETURN NULL;
