...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `Dvlp_Init_V_Question` AS select `t1`.`initId` AS `InitID`,`t1`.`InitNo` AS `InitNo`,`t3`.`KnowledgeID` AS `KnowledgeID`,`t1`.`title` AS `Question`,`t1`.`titleTxt` AS `QuestionTxt`,`t1`.`choiceA` AS `ChoiceA`,`t1`.`choiceB` AS `ChoiceB`,`t1`.`choiceC` AS `ChoiceC`,`t1`.`choiceD` AS `ChoiceD`,`t1`.`answer` AS `Answer`,`t1`.`analysis` AS `QuestionAnalyze`,`t1`.`difficulty` AS `Difficulty`,`t1`.`id` AS `FromQuestionID` from ((`QuestionBankCrawler`.`tb_question` `t1` left join `QuestionBankCrawlerDvlp`.`tb_knowledge` `t2` on(((concat(`t1`.`phase`,`t1`.`subject`) = `t2`.`subject_name`) and (concat(`t1`.`grade`,`t1`.`booklet`) = `t2`.`grade_name`) and (`t1`.`unit` = `t2`.`unit_name`) and (`t1`.`chapter` = `t2`.`chapter_name`) and (`t1`.`knowledge` = `t2`.`knowledge_name`)))) left join `QuestionBankTemp`.`t_ITN_Knowledge` `t3` on((`t2`.`id` = `t3`.`FromCrawlerKnowledgeID`))) where (`t1`.`bookVersion` = '人教版');
