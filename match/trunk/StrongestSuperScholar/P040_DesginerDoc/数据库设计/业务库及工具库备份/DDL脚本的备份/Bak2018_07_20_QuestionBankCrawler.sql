...
CREATE TABLE `tb_question`  (
  ...
  `knowledgeID` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '知识点的ID',
...
-- ----------------------------
-- View structure for Dvlp_Init_V_QuestionSimaple
-- ----------------------------
DROP VIEW IF EXISTS `Dvlp_Init_V_QuestionSimaple`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `Dvlp_Init_V_QuestionSimaple` AS select `T1`.`initId` AS `initId`,`T1`.`id` AS `id`,`T1`.`InitNo` AS `InitNo`,`T1`.`title` AS `title`,`T1`.`titleTxt` AS `titleTxt`,`T1`.`choiceA` AS `choiceA`,`T1`.`choiceB` AS `choiceB`,`T1`.`choiceC` AS `choiceC`,`T1`.`choiceD` AS `choiceD`,`T1`.`answer` AS `answer`,`T1`.`analysis` AS `analysis`,`T1`.`phase` AS `phase`,`T1`.`subject` AS `subject`,`T1`.`grade` AS `grade`,`T1`.`booklet` AS `booklet`,`T1`.`unit` AS `unit`,`T1`.`chapter` AS `chapter`,`T1`.`knowledge` AS `knowledge`,`T1`.`difficulty` AS `difficulty`,`T1`.`bookVersion` AS `bookVersion`,`T1`.`status` AS `status`,`T1`.`currentTime` AS `currentTime` from `tb_question` `T1`;
